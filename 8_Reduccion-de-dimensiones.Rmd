...
```{r }
##modela con la distribución por Poisson, que se ajusten a una distribucion normal
# variance modelling
set.seed(1001)
dec.pbmc <- modelGeneVarByPoisson(sce.pbmc)
#10 % de los genes que estan mas lejos de la curva de tendencia, los que tienen el componente de variación biológico
...
```{r , warning=FALSE, message=FALSE}
##Top zeisel son los genes de HVGs de zeisel
library(scran)
library(scater)
set.seed(100)
...
##elbow
percent.var2 <- attr(reducedDim(sce.pbmc), "percentVar")
chosen.elbow <- PCAtools::findElbowPoint(percent.var2)
plot(percent.var2, xlab = "PC", ylab = "Variance explained (%)")
abline(v = chosen.elbow, col = "red")

choices2 <- getClusteredPCs(reducedDim(sce.pbmc))
chosen.clusters2 <- metadata(choices2)$chosen
plot(choices2$n.pcs, choices2$n.clusters,
    xlab = "Number of PCs", ylab = "Number of clusters"
)
abline(a = 1, b = 1, col = "red")
abline(v = chosen.clusters2, col = "grey80", lty = 2)
...
```{r }
#Ruido tecnico demasiado bajo por el numero de dimensiones tan alto que es 50
set.seed(001001001)
...
```{r, fig.width = 21 }
##t-sne no es deterministico, son graficas exploratorias
set.seed(100)
...
```{r, fig.dim = c(6, 4) }
##N-neighbors numero mayor crea grupos mas grandes y compactos
###no es deterministico, solo exploratorio
set.seed(100)

...
```{r}
# Build graph using k = 10 nearest neighbours in PCA-space
g <- buildSNNGraph(sce.pbmc, k = 15, use.dimred = "PCA")
# Identify communities using the Walktrap method
clust <- igraph::cluster_walktrap(g)$membership
```
...
####Ejercicio###############################################################
#suma de la diagonal
suma <- sum(ratio[1,1]+ ratio[2,2] + ratio[3,3] + ratio[4,4] + ratio[5,5] + ratio[6,6] + ratio[7,7] + ratio[8,8] + ratio[9,9] + ratio[10,10] + ratio [11,11] + ratio[12,12] + ratio[13,13] + ratio[14,14] + ratio[15,15] + ratio[16,16])
...
# identified as memory T-cells (cluster 10).
memory <- 10
